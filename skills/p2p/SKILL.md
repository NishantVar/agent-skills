...
- **my_title**:
  ...
  Default: "".
- **peer**:
  Cmux tab title of the destination peer. First-contact routing key, scoped by workspace/window modifiers and by the caller's workspace by default. By convention, the first word of an incoming message is treated as the peer's cmux tab title.
  Default: "".
- **workspace**:
  Optional workspace scope for first contact or stale-surface recovery. Use a workspace title, `workspace:N`/UUID, or `all`. Omit to default to the caller's workspace unless --window is the only scope modifier.
  Default: "".
- **window**:
