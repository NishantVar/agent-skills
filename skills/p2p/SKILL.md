...
10. `self_title_conflict` → this surface is already registered under one title (`current_title`) but an inbound bootstrap suggested a different one (`suggested_title`). p2p will NOT silently adopt it — a bootstrap is routed by tab title and can land on the wrong pane. Decide from your own role: (a) if you were genuinely RE-TASKED to `suggested_title`, rerun with `--my-title <suggested_title>` to re-register (your prior title is kept in former_titles so peers addressing it still bridge); (b) if this bootstrap was MISROUTED to you and you still hold `current_title`, do NOT register — reply to the sender with a short misroute notice naming your actual title.
11. `empty_message` → write a non-empty body and rerun.
