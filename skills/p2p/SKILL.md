...
- **my_title**:
  ...
  Default: "".
- **peer**: Cmux tab title of the destination peer. Single routing key, scoped to the caller's workspace by default. By convention, the first word of an incoming message is treated as the peer's cmux tab title. Default: "".
...
1. `peer_unknown` → invoke the tfork skill and have it spawn an agent whose first user-turn input is the contents of `payload_file`. Use whatever delayed-input mechanism tfork currently exposes — p2p does not prescribe a flag. The payload file is already written 0600; do not re-create it. If the handoff carries a `workspace` field (non-null), pass `--workspace <value>` to tfork too so the spawned peer lands in the workspace the original send targeted.
