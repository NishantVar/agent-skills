...
def build_bootstrap(peer_name: str, peer_surface: str,
    ...
    body = (
        ...
        f"{suggest_line}"
        f"Please load the p2p skill, register yourself with a stable "
        f"short name (use the suggested name above, or your cmux tab "
        f"title, when you do not already have one), and reply when "
        f"ready."
    )
    if first_message and first_message.strip():
        body += f"\n\nFirst message from {peer_name}: {first_message}"
    return body
