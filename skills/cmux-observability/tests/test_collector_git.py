...
def test_commit_counts_for_authorized_email(tmp_git_repo):
    now = datetime.now()
    counts = commit_counts(tmp_git_repo, authors=["alice@example.com"], now=now)
    assert counts["today"] == 1
    assert counts["week"] == 2
    assert counts["30d"] == 3


def test_commit_counts_excludes_non_authors(tmp_git_repo):
    ...
    now = datetime.now()
    alice = commit_counts(tmp_git_repo, authors=["alice@example.com"], now=now)
    bob = commit_counts(tmp_git_repo, authors=["bob@example.com"], now=now)
...
def test_commit_counts_empty_repo_returns_zeros(tmp_path):
    repo = tmp_path / "empty"
    repo.mkdir()
    subprocess.run(["git", "init", "-q"], cwd=repo, check=True, capture_output=True)
    counts = commit_counts(repo, authors=["alice@example.com"], now=datetime.now())
    assert counts == {"today": 0, "week": 0, "30d": 0}


def test_productivity_resolves_default_author_from_git_config(tmp_git_repo):
    now = datetime.now()
    ...
    prod = productivity([tmp_git_repo], cfg, now=now)
    assert prod.totals["today"] == 1
    assert prod.repos[0].path == str(tmp_git_repo)


def test_productivity_skips_repo_without_user_email(tmp_path):
    ...
    prod = productivity([repo], cfg, now=datetime.now())
    assert prod.repos == []
